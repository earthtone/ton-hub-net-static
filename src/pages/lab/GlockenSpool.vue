...
import { scale as scaleData } from '@/lib/sound.helper'
...
export default {
  ...
  setup () {
    const points = value([])
    const palette = value(random.pick(palettes))
    const audio = value(null)
    const scale = value(scaleData)
    const makePoints = makeGrid().filter(() => random.value() > 0.5).map(points => ({
      ...points,
